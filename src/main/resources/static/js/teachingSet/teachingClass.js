...
//根据gradecode更换成对应的年级名
function changeto(gradecode) {
    var gradename;
    if(gradecode=="01"){
        gradename="一年级"
    }else if(gradecode=="02"){
        gradename="二年级"
    }else if(gradecode=="03"){
        gradename="三年级"
    }else if(gradecode=="04"){
        gradename="四年级"
    }else if(gradecode=="05"){
        gradename="五年级"
    }else if(gradecode=="06"){
        gradename="六年级"
    }else if(gradecode=="07"){
        gradename="七年级"
    }else if(gradecode=="08"){
        gradename="八年级"
    }else if(gradecode=="09"){
        gradename="九年级"
    }else gradename=gradecode
    return gradename;
}
            ...
            if (result.code == "200") {
                ...
                if (gradeList != null && gradeList.length > 0) {
                    ...
                    for (var i = 0; i < gradeList.length; i++) {
                        ...
                        var gradename=changeto(item.gradeCode);
                         items.push('<label><input type="radio" name="gradeId" class="radio" value="' + item.gradeId + '"><i>' + gradename + '</i></label>');
...
function appendDataClass(result) {
    ...
    if (result.data != null && result.data.length > 0) {
        ...
        $.each(result.data, function (idx, t) {
            ...
            html += '<td>' +result.data[idx].subjectid+'</td>'
